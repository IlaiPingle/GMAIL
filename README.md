...
[URL Bloom Filter Project.pdf](https://github.com/user-attachments/files/20013274/URL.Bloom.Filter.Project.pdf)

## Extensibility & Open/Closed Principle

### Renaming Commands
In the previous implementation the `InputManager` parsed lines like `POST X` or `GET Y` and directly invoked the corresponding function. Renaming a command (for example, from `POST` to `PUBLISH`) forced us to modify only the parsing logic inside `InputManager`, while the rest of the system remained untouched.
...
Previously, `InputManager` treated every incoming line—whether from the console or a socket—as a raw string to parse and dispatch. Swapping the input transport (console ↔️ socket) never required touching command-dispatch or business logic.
