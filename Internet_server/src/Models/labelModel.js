const LabelsRepository = new Map();

// This variable is used to generate unique IDs for each label
let nextLabelId = 1;

/**
 * Creates a new label for a user
 * @param {number} userId - The ID of the user creating the label
 * @param {string} name - The name of the label
 * @param {string} color - The color code for the label (optional)
 * @returns {Object} The created label object
 */
function createLabel(userId, name, color = "#808080") {
    const newLabel = {
        id: nextLabelId++,
        name,
        color,
    ...
    if (!LabelsRepository.has(userId)) {
        LabelsRepository.set(userId, []);
    }
    LabelsRepository.get(userId).push(newLabel);
    return newLabel;
}
